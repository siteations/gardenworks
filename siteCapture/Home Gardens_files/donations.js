...
$(document).ready(function(){
  console.log('got here on donations pre-obj');
  ...
  (function(){
    ...
    this.triggerAlert = function(alertMessage, insertBefore) {
      var message = '<div class="form-error beet">'+alertMessage+'</div>';
      insertBefore.after(message);
      this.scrollContainerTop();
    }

    this.removeAlerts = function() {
      $('.form-error.beet').remove();
      $('.error').each(function(){
  ...
  if ($('.progress-indicator-stages').length) {
    $('.progress-stage.stage-2').hide();
   $('.progress-stage.stage-3').hide();
    ...
    $('.progress-indicator-stages .stage-label').each(function(){
    ...
    $('.progress-stage-button-next').each(function(){$(this).click(function(event){
      ...
      if (isValid) {
        ...
        progressIndicator.find('.active').removeClass('completed');
    ...
    progressStages.find('input[type="submit"]').click(function(event){
      progressIndicator.find('.active').addClass('completed');
    });
